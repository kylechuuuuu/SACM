...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        if self.use_adapter_skip and adapter_features is not None:
            adapter_stack = torch.stack(adapter_features, dim=0)
            ...
            for i, (adapter_feat, weight) in enumerate(zip(adapter_features, adapter_weights)):
                weighted_adapter_features += adapter_feat * weight.unsqueeze(-1).unsqueeze(-1)
            
            processed_adapter = self.adapter_aggregator(weighted_adapter_features)
            
            processed_adapter = processed_adapter.permute(0, 3, 1, 2)
            if processed_adapter.shape[2:] != upscaled_embedding.shape[2:]:
